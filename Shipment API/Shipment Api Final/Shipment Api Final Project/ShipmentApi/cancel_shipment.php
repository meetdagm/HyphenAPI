...
$reason = $url->getHttpHeaderData('reason');
//endregion

//region attempt to delete
if ($Shipment_id != "" && $reason != "") {
    $shipmentDeliveryMan->cancelShipment($Shipment_id,$reason);
} else {
    array_push($response, array("server_response" => "cancel_shipment_error",
        "error" => "You must specify the shipment "));
    echo json_encode($response);
    exit();
}
//endregion